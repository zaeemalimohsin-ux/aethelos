...
describe("Adversarial Economics > Flash Loan Vote Stacking", () => {
  test("prevents an attacker from passing a proposal by passing the same 1 point between sybils", async () => {
    ...
    const sybils = await Promise.all(Array.from({ length: 5 }).map(() => generateKeyPair()));
    ...
    const addEvent = async (payload: SignedEvent["payload"], author: Awaited<ReturnType<typeof generateKeyPair>>) => {
      lamport++;
    ...
    await addEvent({ type: "genesis", initialPoints: "100", cellName: "Flash Bank" }, founder);

    // 2. Founder transfers 20 points to Attacker1
    await addEvent({ type: "invite", invitee: attacker1.publicKeyHex }, founder);
    await addEvent({ type: "proposal_vote", proposalId: `admit:${attacker1.publicKeyHex}`, approve: true }, founder);
    await addEvent({ type: "accept_invite", inviter: founder.publicKeyHex }, attacker1);
    await addEvent({ type: "transaction", to: attacker1.publicKeyHex, amount: "20" }, founder);

    // 3. Founder invites the 5 Sybils
    for (const sybil of sybils) {
      await addEvent({ type: "invite", invitee: sybil.publicKeyHex }, founder);
      await addEvent({ type: "proposal_vote", proposalId: `admit:${sybil.publicKeyHex}`, approve: true }, founder);
