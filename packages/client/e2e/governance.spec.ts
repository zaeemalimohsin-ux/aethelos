...
test.describe('Governance & Fixes', () => {
  ...
  test('Alice expels Bob', async () => {
    ...
    await alicePage.locator('.card', { hasText: 'Propose something' }).getByLabel('About who?').selectOption(bobPubkey);
    await alicePage.click('button:has-text("Start proposal")');

    // Alice approves the expel proposal
    await alicePage.locator('.card', { hasText: 'Remove member' }).locator('button:has-text("Approve")').click();
    ...
    await expect(alicePage.locator(`text="${bobPubkey.slice(0, 12)}…"`)).not.toBeVisible({ timeout: 15000 });
  });
});
