...
test.describe('Tauri E2E', () => {
  ...
  test.beforeAll(() => {
    ...
    try {
      const appData = path.join(os.homedir(), 'AppData', 'Roaming', 'org.aethelos.desktop');
      const localAppData = path.join(os.homedir(), 'AppData', 'Local', 'org.aethelos.desktop');
  ...
  test('Tauri E2E: Genesis flow and navigating views', async ({ browser }) => {
    ...
    try {
      await expect(page.locator('text="Create a new identity"').or(page.locator('text="Use a different identity"')).first()).toBeVisible({ timeout: 15000 });
    } catch (e) {
    ...
    if (await page.locator('text="Use a different identity"').isVisible()) {
      ...
      await expect(page.locator('text="Create a new identity"')).toBeVisible({ timeout: 5000 });
    }
    ...
    await expect(page.locator('.app-header')).toContainText('AethelOS', { timeout: 30000 });
