...
test.describe('AethelOS Multi-Peer Mesh (3 Peers)', () => {
  ...
  test('Alice starts, Bob joins via Alice, Charlie joins via Bob', async () => {
    ...
    await alicePage.locator('.card', { hasText: 'Send Value' }).locator('select').selectOption(bobPubkey);
    await alicePage.locator('.card', { hasText: 'Send Value' }).getByLabel('Amount (Value)').fill('1000');
    await alicePage.locator('.card', { hasText: 'Send Value' }).locator('button:has-text("Send")').click();
    ...
    // Bob approves
    await bobPage.locator('.card', { hasText: 'Admit member' }).locator('button:has-text("Approve")').click();
    ...
    // Charlie is approved!
    await expect(charliePage.locator('.app-main')).toContainText("You're approved — welcome in!", { timeout: 30000 });
    await charliePage.click('button:has-text("Accept invitation")');
    await expect(charliePage.locator('button:has-text("Invite people")')).toBeVisible({ timeout: 30000 });
  });
});
