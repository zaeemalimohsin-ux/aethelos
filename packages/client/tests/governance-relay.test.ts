...
describe("governance relay across namespaces", () => {
  ...
  it(
    ...
    async () => {
      ...
      await mirror.syncLinkedNamespaces(
        childPool,
        childHead,
        [wsUrl],
      ...
      const { state: reducedParent, rejected } = reduceEventsWithAudit(
        parentNs,
        parentEvents,
      );
      expect(
        rejected.filter((r) => r.eventType === "relay_cell_governance"),
      ).toHaveLength(0);
      expect(reducedParent.bridges).toContain(childHead.publicKeyHex);
      expect(reducedParent.parameters.decay_rate).toBeGreaterThan(
        DEFAULT_PARAMETERS.decay_rate,
      );
