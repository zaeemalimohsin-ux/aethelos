...
| P3.2 | Expulsion → highest Pool escrow + slider prune | `superstructure.test.ts`, `governance-fixes.test.ts` (slider rows/columns pruned on expel) | Covered | Hop-by-hop escalation via bridge release when parent unknown |
...
| P4.2 | Liquid vouch / Head election + recall | `core.test.ts` (head recall), E2E `community-scale.spec.ts` head shift; UI threshold in `GovernanceView` | Covered | Headless state when no challenger crosses threshold |
...
- **Deploy-path E2E:** CI runs localhost vite+relay (`e2e` job) plus `docker-founder` (same-origin `/ws`); share-URL specs are Windows/env-gated via `proof:product`.
